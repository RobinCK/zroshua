...
export default function JournalPage({ tick }: { tick: number }) {
  ...
  const targetData = useMemo(
    () => [
      { group: 'Zones', items: (zones ?? []).map((z) => ({ value: `zone:${z.id}`, label: z.name })) },
      { group: 'Groups', items: (groups ?? []).map((g) => ({ value: `group:${g.id}`, label: g.name })) },
    ],
    [zones, groups],
  );
    ...
    <Stack>
      ...
      <Group gap="xs" grow wrap="wrap">
        <Select
          ...
          data={Object.keys(KIND_COLORS).map((k) => ({ value: k, label: KIND_LABELS[k] ?? k }))}
          value={kind}
          onChange={setKind}
          clearable
        />
                    ...
                    <Table.Td>
                      {t ? (
                        <Group gap={6} wrap="nowrap">
                          <Badge size="xs" variant="dot" color={t.color}>
                            {t.tag}
                          </Badge>
      ...
      {!filtered.length && <Text c="dimmed">No matching journal entries.</Text>}
    </Stack>
  );
}
